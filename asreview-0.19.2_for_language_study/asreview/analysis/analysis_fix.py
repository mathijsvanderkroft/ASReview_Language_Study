...
class Analysis():
    ...
    def _get_inc_found(self, labels=False):
        ...
        # find length of longest list in inclusions_found
        max_len = max([len(inc) for inc in inclusions_found])
        # find max value of each list in inclusions_found
        max_inc = [max(inc) for inc in inclusions_found]
        # pad all lists to the same length with max_value
        for i, inc in enumerate(inclusions_found):
            inc.extend([max_inc[i]] * (max_len - len(inc)))
